...
const EditTaskModal = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        console.log(status)
        if (status.length > 0)
            dispatch(updateTask({ title, description,deadline,priorityLevel, status, id }))
        else
            dispatch(updateTask({ title, description,deadline,priorityLevel, id }))
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose}>
            ...
            <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
                ...
                <div className='flex flex-col'>
                    <label htmlFor="description" className='text-sm font-semibold'>Deadline of Task</label>
                    <input
                        type="text"
                        name="deadline" 
                        id="deadline"
                ...
                <div className='flex flex-col'>
                    <label htmlFor="description" className='text-sm font-semibold'>Priority Leavel of Task</label>
                    <input
                        type="text"
                        name="priorityLevel"
                        id="priorityLevel"
